...
from wasserstein_novelty import *
from utils import *
from gcn import *
...
val_structs   = read_structure_from_csv("val.csv")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("imgs/novelty_vs_vacancies.png", dpi=300)
plt.show()
...
for swa in swaps:
    pert = perturb_structures_corrupt(val_structs, vacancy_prob=0., swap_prob=swa)
    score, *_ = scorer.compute_novelty(pert)
...
plt.grid(True)
plt.tight_layout()
plt.savefig('imgs/novelty_vs_swaps.png', dpi=300)
plt.show()
...
for sig in sigmas:
    pert = perturb_structures_gaussian(val_structs, sigma=sig, teleport_prob=0.)
    score, *_ = scorer.compute_novelty(pert)
...
plt.grid(True)
plt.tight_layout()
plt.savefig('imgs/novelty_vs_sigma.png', dpi=300)
plt.show()
...
for tele in teles:
    pert = perturb_structures_gaussian(val_structs, teleport_prob=tele, sigma=0.)
    score, *_ = scorer.compute_novelty(pert)
...
plt.grid(True)
plt.tight_layout()
plt.savefig('imgs/novelty_vs_teleport.png', dpi=300)
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig('imgs/novelty_vs_shared.png', dpi=300)
plt.show()
